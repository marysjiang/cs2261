...
void drawRect4(int col, int row, int width, int height, volatile unsigned char colorIndex) {
    ...
    for (int i = 0; i < height; i++) {
        if (col % 2 == 0) {
            if (width % 2 == 0) {
                DMANow(3, &colorData,
                    &videoBuffer[((row + i)*(240)+(col)) / 2], (2 << 23) | (width / 2));
            } else {
                if (width > 1) {
                    DMANow(3, &colorData, &videoBuffer[((row + i)*(240)+(col)) / 2],
                        (2 << 23) | ((width - 1) / 2));
                }
            ...
            if (width % 2 == 1) {
                ...
                if (width > 1) {
                    DMANow(3, &colorData, &videoBuffer[((row + i)*(240)+(col + 1)) / 2],
                        (2 << 23) | (width / 2));
                }
            } else {
                setPixel4(col, row + i, colorIndex);
...
void drawImage4(int col, int row, int width, int height, const unsigned short *image) {
    ...
    for (int i = 0; i < height; i++) {
        DMANow(3, &image[((i)*(width)+(0)) / 2], &videoBuffer[((row + i)*(240)+(col)) / 2], width / 2);
    }
}
