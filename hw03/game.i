...
void updatePlayer() {
    ...
    if ((player.col + player.width + 2 <= 240) && ((!(~(oldButtons) & ((1<<4)))) && (~(buttons) & ((1<<4))))) {
        player.col += player.cdel;
    }

    if ((player.col + player.width + 2 <= 240) && (~((*(volatile unsigned short *)0x04000130)) & ((1<<4)))) {
        player.col += player.cdel;
    }


    if ((player.col - 2 >= 0) && ((!(~(oldButtons) & ((1<<5)))) && (~(buttons) & ((1<<5))))) {
        player.col -= player.cdel;
    }

    if ((player.col - 2 >= 0) && (~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))) {
        player.col -= player.cdel;
    }
}
...
void drawBall(BALL* b) {
    if (b->active) {
        drawRect(b->oldCol, b->oldRow, b->width, b->height, 0);
        drawRect(b->col, b->row, b->width, b->height, ((31) | (0)<<5 | (31)<<10));
    } else {
        drawRect(b->oldCol, b->oldRow, b->width, b->height, 0);
        b->active = 0;
    }
